...
export default class CustomCreatableSelect extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  filterValues = (data, value) => {
    ...
    if (value && value !== '') {
      ...
      if (data.label.search(regExp) === -1) isDisplayed = false;
    }

    if (isDisplayed) this.optionsCounter++;
    if (this.props.options.length === this.optionsCounter) this.optionsCounter = 0
    return isDisplayed;
  }
